...
class MoveMemento implements Memento {
    ...
    MoveMemento(Board boardIn, Command cmIn) {
        board = boardIn;
        cm = cmIn;
        // Save the state of the pieces BEFORE the move happens
        fromPieceName = board.getICCF(cm.getFrom());
        toPieceName = board.getICCF(cm.getTo());
    }

    int getFrom() { return cm.getFrom(); }
    int getTo() { return cm.getTo(); }
    String getFromPiece() { return fromPieceName; }
    String getToPiece() { return toPieceName; }
