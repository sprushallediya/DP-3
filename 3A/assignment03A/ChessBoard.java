...
import static assignment03A.SquareColor.CLICKED;
		...
		l.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				if(fromClick) {
					...
					if(board.getICCF(toICCF).getColor() != SquareColor.NONE)
	...
	public static JPanel drawGui(JFrame frame, Board board) {
		...
		for (int r = 8; r >= 1; r--) {
			...
			for (int f = 1; f <= 8; f++) {
				...
				if (piece.getColor() == SquareColor.NONE) {
					addColoredUnicodeCharToContainer(
							" ", gui, new Color(203,203,197),
							Color.DARK_GRAY,
							frame, board, f, r);
				} else {
					if(piece.getColor() == SquareColor.WHITE)
						addColoredUnicodeCharToContainer(
